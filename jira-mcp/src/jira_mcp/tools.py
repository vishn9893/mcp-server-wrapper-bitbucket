...
async def search_issues(c: JiraClient, jql: str, max_results: int = 50, fields: list[str] | None = None) -> Any:
    ...
    return await c.post("/search/jql", json=payload)
...
async def create_issue(c: JiraClient, project_key: str, issue_type: str, summary: str, description: str | None = None, extra_fields: dict[str, Any] | None = None) -> Any:
    ...
    if description is not None: fields["description"] = _adf(description)
...
async def add_comment(c: JiraClient, issue_key: str, body: str) -> Any: return await c.post(f"/issue/{issue_key}/comment", json={"body": _adf(body)})
...
async def assign_issue(c: JiraClient, issue_key: str, account_id: str | None = None) -> Any: return await c.put(f"/issue/{issue_key}/assignee", json={"accountId": account_id})
