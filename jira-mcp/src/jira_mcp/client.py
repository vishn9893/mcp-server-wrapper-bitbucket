...
class JiraClient:
    ...
    async def __aenter__(self) -> "JiraClient":
        if self._http is None:
            self._http = httpx.AsyncClient(base_url=self.settings.api_url, auth=(self.settings.email, self.settings.token), verify=self.settings.verify_tls, timeout=self.settings.timeout, follow_redirects=True)
        return self
